...
exports.readFileName = ({ fileName, selectedText }) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    let inputDirectoryPath = {
        [LANGUAGE_SUFFIX.PUG]: config.get("pug-input-directory") || "",
        [LANGUAGE_SUFFIX.LESS]: config.get("less-input-directory") || "",
    };
    ...
    let orange = vscode.window.createOutputChannel("COMPILE HERO");

    if (!compileStatus[fileSuffix])
        return;
	orange.appendLine(fileName);
   	let newFile = fileName.replace(inputDirectoryPath[fileSuffix], outputDirectoryPath[fileSuffix])
	let outputPath = path.dirname(newFile)
	orange.appendLine( inputDirectoryPath[fileSuffix]);
	orange.appendLine(newFile);	orange.appendLine( outputDirectoryPath[fileSuffix]);
	orange.appendLine(outputPath);
    orange.appendLine(fileSuffix);
